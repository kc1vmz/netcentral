...
public class NetQuestionReport {
    ...
    private Table addDocumentHeader(String taskId) {
        ...
        Cell cell1 = new Cell().add(new Paragraph("Q & A LOG")).setBorder(new SolidBorder(1)).setVerticalAlignment(VerticalAlignment.MIDDLE);
    ...
    private Table addCommunicationsLogHeader() {
        ...
        UnitValue tiny = new UnitValue(2, value*5);
        UnitValue small = new UnitValue(2, value*10);
        UnitValue medium = new UnitValue(2, value*15);
        UnitValue large = new UnitValue(2, value*70);
        Cell cell1 = new Cell().add(new Paragraph("Q/A")).setBorder(new SolidBorder(1)).setTextAlignment(TextAlignment.CENTER).setWidth(tiny);
        PdfUtils.fixupFontBold(cell1);
        table.addCell(cell1);

        Cell cell2 = new Cell().add(new Paragraph("TIME")).setBorder(new SolidBorder(1)).setTextAlignment(TextAlignment.CENTER).setWidth(small);
        PdfUtils.fixupFontBold(cell2);
        table.addCell(cell2);

        Cell cell3 = new Cell().add(new Paragraph("FROM")).setBorder(new SolidBorder(1)).setTextAlignment(TextAlignment.CENTER).setWidth(medium);
        PdfUtils.fixupFontBold(cell3);
        table.addCell(cell3);

        Cell cell4 = new Cell().add(new Paragraph("TEXT")).setBorder(new SolidBorder(1)).setTextAlignment(TextAlignment.CENTER).setWidth(large);
     ...
     */
    private void addCommunicationLogRows(Table table, List<NetQuestionReportItem> messages, int page) {
        table.useAllAvailableWidth();
        ...
        for (NetQuestionReportItem message : messages) {
            ...
            String timeStr = DateStrUtils.getTimeStr(message.getTime());

            Cell cellLabel = new Cell().add(new Paragraph((String.format("%s", message.getLabel()))));
            Cell cellTime = new Cell().add(new Paragraph(timeStr));
            Cell cellFrom = new Cell().add(new Paragraph((String.format("%s", message.getCallsign()))));
            Cell cellMsg = new Cell().add(new Paragraph((String.format("%s", message.getText()))));
    ...
    private Table addFooter(int pageCurrent, int pageCount) {
        ...
        Cell cell = new Cell().add(new Paragraph("Net Q&A Report")).setBorder(new SolidBorder(1)).setTextAlignment(TextAlignment.RIGHT);
    ...
    private Table addGenerator() {
        Table table = new Table(1);
        table.useAllAvailableWidth();
        String text = String.format("Net Q&A Report generated by %s %s", SoftwareIdentity.NAME, SoftwareIdentity.VERSION);
