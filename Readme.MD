...
QR n - Report on question n and its answers
INV callsign[,callsign2,...] - invite one or more callsigns to a net  
NCI - toggle notification of all check-ins and check-outs (default OFF)
H - display help message of all commands
  
CI, CO, T, F, X, E, B, NCI and Y can all be combined in a single message (ex: CI X 50 E COMM B SO NCI)
...
To utilize a map caching service, allowing you to be completely disconnected from the Internet for operations, update the Net Central UI .env file with the URL to access map tiles:
...
Java 21 JDK is preferred for build, but a JRE based on Java 17 for execution.  
...
Java 17 or higher JRE must be installed and configured prior to running any Net Central service.
...
### Running Net Central Server

To run the JAR file, specify:  
    java -Dmicronaut.environments=mysql -jar netcentral-server-1.0.24.jar
or
    java -Dmicronaut.environments=h2 -jar netcentral-server-1.0.24.jar

You must choose between mysql (an external MySQL database running locally or remotely) or h2 (an Sqlite embedded database).

### Running Net Central Transceiver Modules

transceiver-agw-1.0.24.jar  
transceiver-aprsis-1.0.24.jar  
transceiver-kenwood-1.0.24.jar  
transceiver-kiss-1.0.24.jar  
transceiver-test-1.0.24.jar  
